...
# Check target chain
tmp_file_chains = target_ppi_pair_id.split("_")
if target_chain == tmp_file_chains[1]:
    target_pid = "p1"
elif target_chain == tmp_file_chains[1]:
    target_pid = "p2"
...
"""
Second method to try docking, based on masif-search second_stage_alignment. 
"""
def second_stage_align (
    desc_dir,
    surf_dir,
    target_desc,
    target_pcd,
    ...
    for pid in pids:
        ...
        source_desc = np.load(
            os.path.join(desc_dir, source_name, pid + "_desc_straight.npy")
        )
        source_cloud = read_point_cloud(
                os.path.join(surf_dir, "{}.ply".format(pdb_id + "_" + chain))
        )
        source_patch_coords = subsample_patch_coords(pdb, pid, precomp_dir)

        # Read the point with the highest shape compl.
        source_labels = np.load(os.path.join(precomp_dir, target_pdb, target_pid + "_sc_labels.npy"))
        center_point = np.argmax(np.median(np.nan_to_num(sc_labels[0]), axis=1))

        # This is where the descriptors are actually compared (stage 1 of the MaSIF-search protocol)
...
# Second match, compare descriptors
